...
실측 결과 (정확 모드): 이론 절감 27.9% 가

```
   워드폭   batch          compaction
      1    3.4% (12%)     27.9% (100%)    <- 현 채택 설정
     32    3.8% (14%)     26.6%  (95%)
```

**워드폭만으로 정해지지 않는다.** `batch` 열은 워드폭을 줄여도 3.4~3.8% 로
평평하다 — 묶음(`batch_size`)이 건너뛰기 단위를 정하므로 워드폭이 개입할
여지가 없다. 즉 실현 절감은 **워드폭과 스케줄 정책이 함께** 정한다.
