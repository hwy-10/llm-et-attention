...
입력 토큰 하나의 임베딩 `x` 에서 세 개의 벡터를 만든다.
...
```
① 점수      S = Q · K^T / √d          ← ★ 이 프로젝트의 범위
② 마스킹    미래 토큰 가림 (causal)
③ 정규화    A = softmax(S)
④ 가중합    O = A · V
```

`d` 는 head 차원(이 프로젝트에서는 64)이고, `√d` 로 나누는 것은
차원이 커질수록 내적 값이 커져 softmax 가 한쪽으로 쏠리는 것을 막기 위한 것이다.
...
* ④ 가중합은 이미 정해진 비율대로 더하는 것이라 건너뛸 대상이 없다.
* 반면 ①은 **점수가 작을 것이 확실한 토큰**을 계산 도중에 알아낼 수 있다.

그리고 softmax 는 지수함수라서 **점수가 조금만 낮아도 가중치가 급격히 0에 가까워진다.**
그래서 상위 몇 개 토큰이 출력의 대부분을 결정하고, 나머지는 계산해도 거의 버려진다.
이 편중이 조기 종단이 성립하는 근거다.
